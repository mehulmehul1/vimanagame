// g2p.wgsl - Grid to Particle with Sphere Constraint
// ==================================================
// Updates particle velocities from grid and applies sphere constraint for tunnel effect.
// Based on: https://github.com/matsuoka-601/WaterBall/blob/master/mls-mpm/g2p.wgsl
...
fn g2p(@builtin(global_invocation_id) id: vec3<u32>) {
    if (id.x < numParticles) {
        ...
        // Sphere constraint - creates hollow tunnel effect
        ...
        // Push particles toward sphere surface (creates hollow center)
        if (dot(dist, dist) < r * r) {
            particles[id.x].v += -(r - sqrt(dot(dist, dist))) * dirToOrigin * 3.0;
        }

        // Gentle attraction to center keeps particles constrained
